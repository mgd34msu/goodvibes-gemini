...
var PLUGIN_ROOT = process.env.GEMINI_PLUGIN_ROOT || path.resolve(__dirname, "../../../");
var PROJECT_ROOT = process.env.GEMINI_PROJECT_DIR || process.cwd();
...
function logError(context, error) {
  ...
  console.error(`[GoodVibes ${timestamp2}] ERROR in ${context}: ${message}`);
  if (stack) {
    console.error(stack);
  }
}
...
function getPluginVersion() {
  try {
    const manifestPath = path16.join(PLUGIN_ROOT, "package.json");
...
async function injectSettings(cwd, pluginRootOverride) {
  ...
  const geminiDir = path17.join(cwd, ".gemini");
  const settingsPath = path17.join(geminiDir, "settings.json");
  try {
    const settingsExist = await fileExists(settingsPath);
    if (!settingsExist) {
      const geminiDirExists = await fileExists(geminiDir);
      if (!geminiDirExists) {
        await fs13.mkdir(geminiDir, { recursive: true });
...
async function injectProjectSettings(projectDir) {
  try {
    ...
    if (result.success) {
      if (result.created) {
        debug("Created .gemini/settings.json with GoodVibes hooks");
      } else if (result.hooksAdded) {
        debug("Added GoodVibes hooks to existing .gemini/settings.json");
      } else {
        debug("GoodVibes hooks already present in .gemini/settings.json");
      }
